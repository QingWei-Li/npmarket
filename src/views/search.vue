<template>
  <main class="PageSearch">
    <filter-panel
      ...
      :total="total"
    >
      <spinner class="PageSearch__list__spinner" v-if="loadingList" color="#0084ff" size="30px"/>
    </filter-panel>
    <section class="PageSearch__main">
      ...
      <readme-panel
        ...
        :options="{
          baseUrl: $at('pkg.collected.metadata.links.repository')
        }"
      ></readme-panel>
    </section>
  </main>
...
export default {
  ...
  methods: {
    ...
    async loadInfo(name) {
      ...
      this.pkg = {};
      CACHED[name] = this.pkg = (await this.$npms(
